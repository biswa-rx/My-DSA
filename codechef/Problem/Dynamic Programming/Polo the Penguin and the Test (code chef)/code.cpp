...
const int N = 1e7 + 2;
int dp[105][105];
int dynamicProgramming(int c[],int p[],int t[],int n,int w){
    if(w<0){
        return -c[n+1]*p[n+1];
    }
    if(n<0 || w==0){
    ...
    int m=max(dynamicProgramming(c,p,t,n-1,w-t[n])+c[n]*p[n],dynamicProgramming(c,p,t,n-1,w));
    return dp[n][w]=m;
}
void run()
{
    ...
    cout<<dynamicProgramming(c,p,t,n-1,w)<<endl;
